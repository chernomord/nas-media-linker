...
test("listDir rejects non-ASCII path segments", async () => {
  const fixture = await makeFixture();
  try {
    const result = await fixture.executor.listDir({
      dir: `${fixture.roots.torrents}/Фильм`,
    });
    assert.equal(result.ok, false);
    assert.equal(result.code, 2);
    assert.match(result.stderr, /Non-ASCII or control characters in path/);
  } finally {
    await rm(fixture.base, { recursive: true, force: true });
  }
});
...
test("linkMovie rejects non-ASCII source paths", async () => {
  const fixture = await makeFixture();
  try {
    const result = await fixture.executor.linkMovie({
      src: `${fixture.roots.torrents}/Фильм.mkv`,
      title: "Movie",
      year: "2024",
    });
...
test("linkSeason rejects non-ASCII source paths", async () => {
  const fixture = await makeFixture();
  try {
    const result = await fixture.executor.linkSeason({
      srcDir: `${fixture.roots.torrents}/Сериал`,
      title: "Show",
      season: "1",
      year: "2025",
    });
