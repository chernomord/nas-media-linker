...
| --- | --- | --- | --- |
| Path validation (`src/srcDir` inside `TORRENTS_ROOT`) | covered | [`test/helper-api.test.mjs`](./test/helper-api.test.mjs): `movie endpoint rejects source outside torrents root`, `movie endpoint rejects traversal payload with nested .. segment`, `movie endpoint accepts printable Unicode path under torrents root`, `movie endpoint returns executor-error shape for control characters under torrents root`, `season endpoint rejects source directory outside torrents root`, `season endpoint rejects traversal payload with nested .. segment`, `season endpoint accepts printable Unicode path under torrents root`, `season endpoint returns executor-error shape for control characters under torrents root`, `list endpoint returns executor-error shape for nested traversal under allowed root`, `list endpoint accepts printable Unicode path under allowed root`, `list endpoint returns executor-error shape for control characters under allowed root`; [`test/executor.test.mjs`](./test/executor.test.mjs): `listDir rejects traversal segments`, `listDir allows printable Unicode path segments`, `listDir rejects control characters in path segments`, `linkMovie allows printable Unicode source paths`, `linkMovie rejects control characters in source paths`, `linkSeason allows printable Unicode source paths`, `linkSeason rejects control characters in source paths` | Remaining gaps are deeper normalization/spoofing policy choices, not missing current printable-Unicode/traversal boundary coverage |
| Allowlist for list (`Torrents/Movies/TV only`) | covered | [`test/helper-api.test.mjs`](./test/helper-api.test.mjs): `list endpoint rejects disallowed root with frozen error shape`, `list endpoint rejects sibling path that only shares allowed-root prefix`, `list endpoint accepts printable Unicode path under allowed root`, `list endpoint returns executor-error shape for control characters under allowed root`; route-level `/api/list` success and error variants are now frozen | Remaining gaps are only deeper encoding/normalization policy choices |
...
5. Deeper normalization/spoofing hardening only if we want to move beyond the current printable-Unicode path model.
